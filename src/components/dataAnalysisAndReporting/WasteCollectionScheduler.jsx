...
const WasteCollectionScheduler = () => {
  ...
  useEffect(() => {
    ...
    const fetchHighPriorityBins = async () => {
      ...
      binsSnapshot.forEach((doc) => {
        ...
        if (data.currentLevel > 75) { // If waste level is more than 75%
            console.log('first')
  ...
  return (
    <div>
      ...
      <button onClick={createWasteCollectionRoute}>Create Optimized Route</button>
      
      {/* Google Map */}
      <LoadScript googleMapsApiKey="AIzaSyDsUxChPKhJURlI4ZEeadAadiC0xKeIHew">
        <GoogleMap
          mapContainerStyle={{ width: '100%', height: '400px' }}
          center={highPriorityBins.length > 0 ? { lat: highPriorityBins[0].location.latitude, lng: highPriorityBins[0].location.longitude } : { lat: 6.9271, lng: 79.8612 }} // Default center Colombo
          zoom={10}
        >
